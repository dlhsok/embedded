...
//   RC1702_RST=0;
    RST_PutVal(bitRSTPtr, 0);
   DelayMs(50);
   DelayMs(50);

//   RC1702_RST=1;
   RST_PutVal(bitRSTPtr, 1);
   DelayMs(5);
   DelayMs(50);
   DelayMs(50);
   DelayMs(50);
...
//     RC1702_SCK = 0;
     SCK_PutVal(bitSCKPtr, 0);
//     RC1702_NSS = 0;
     NSS_PutVal(bitNSSPtr, 0);
     ucAddr =((Address<<1)&0x7E)|0x80;

     for(i=8;i>0;i--)
     {
//         RC1702_MOSI = ((ucAddr&0x80)==0x80);
    	 MOSI_PutVal(bitMOSIPtr, (ucAddr&0x80)==0x80);
//         RC1702_SCK = 1;
         SCK_PutVal(bitSCKPtr, 1);
         ucAddr <<= 1;
//         RC1702_SCK = 0;
         SCK_PutVal(bitSCKPtr, 0);
     }
     ...
     for(i=8;i>0;i--)
     {
         SCK_PutVal(bitSCKPtr, 1);
         ucResult <<= 1;
         ucResult|=MISO_GetVal(bitMISOPtr);
         SCK_PutVal(bitSCKPtr, 0);
     }

//	 RC1702_NSS = 1;
     NSS_PutVal(bitNSSPtr, 1);
//	 RC1702_SCK = 1;
     SCK_PutVal(bitSCKPtr, 1);
     return ucResult;
}
